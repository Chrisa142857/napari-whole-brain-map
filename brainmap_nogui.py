...
def main():
    ...
    #         org_overlap_r=OVERLAP_R,
    #         device=device)
    # bm.arrange_brainmap_layer(stitch_type=stitch_type, manual_stitch_path='/cajal/Felix/Lightsheet/stitching/redo_Manual_aligned_L57D855P5')
    # bm.run_fusion(save_root='/export_home/ziquanw/tmp')
    # bm.save_fused_image(save_root=fused_image_save_root)
    bm = Brainmap(cpp_result_root=nis_cpp_results, 
    ...
    #         org_overlap_r=OVERLAP_R,
    #         device=device)
    # bm.arrange_brainmap_layer(stitch_type=stitch_type, manual_stitch_path='/cajal/Felix/Lightsheet/stitching/redo_Manual_aligned_L57D855P2')
    # bm.run_fusion(save_root='/export_home/ziquanw/tmp')
    # bm.save_fused_image(save_root=fused_image_save_root)
...
class Brainmap:
    def __init__(
        ...
        self.zstitch_remap = {}
        for choicei in trange(len(self.tile_list), desc='Loading cell profile raw'):
    ...
    def arrange_brainmap_layer(self, stitch_type='Manual', manual_stitch_path=None, zstitchtype='Avg'):
    ...
    def run_fusion(self, save_root='./tmp'):
        ...
        self.doubled_label = nms_undouble_cell(stack_nis, stack_label, tile_center, self.seg_shape, tile_wh=self.seg_shape, tile_lt_loc=self.cache_tform, overlap_r=self.org_overlap_r, btag=self.btag, device=self.device, save_path=save_root)
        lrange = 1000
        undoubled_center = []
        undoubled_label = []
        undoubled_vol = []
